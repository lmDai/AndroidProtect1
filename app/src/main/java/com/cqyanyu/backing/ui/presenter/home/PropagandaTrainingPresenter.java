...
import com.cqyanyu.backing.ui.fragment.home.FeedBackFragment;
...
public class PropagandaTrainingPresenter extends BasePresenter<PropagandaTrainingView> {
    ...
    public void init() {
        if (getView() != null) {
            ...
            if (InfoManger.getInstance().isPermission("88")) {
                OnlineTrainingFragment onlineTrainingFragment = new OnlineTrainingFragment();
                xFragment.add(onlineTrainingFragment);
                titles.add("在线培训");
            }
            if (InfoManger.getInstance().isPermission("56")) {
                FeedBackFragment feedBackFragment = new FeedBackFragment();
                xFragment.add(feedBackFragment);
                titles.add("举报反馈");
            }
            if (InfoManger.getInstance().isPermission("57")) {
